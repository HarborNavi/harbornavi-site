...
async function initializePilotSurveyTable() {
  ...
  await db`alter table pilot_family_surveys add column if not exists item_scores jsonb not null default '{}'::jsonb`;
  await db`alter table pilot_family_surveys alter column content_score type numeric(5,1) using content_score::numeric`;
  await db`alter table pilot_family_surveys alter column scenario_score type numeric(5,1) using scenario_score::numeric`;
  await db`alter table pilot_family_surveys alter column reliability_score type numeric(5,1) using reliability_score::numeric`;
  await db`alter table pilot_family_surveys alter column total_score type numeric(5,1) using total_score::numeric`;
...
export async function savePilotSurvey(input: PilotSurveyInput) {
  ...
  const rows = await sql()`
    insert into pilot_family_surveys (
      ...
      scheduling_confidence, completion_commitment, visit_comfort, past_participation_level, referral_source, anything_else,
      ...
      score_band, score_version, item_scores, automatic_summary,
      ...
      ${JSON.stringify(application.scenario_frequency)}::jsonb,
      ...
      ${application.visit_comfort}, ${application.past_participation_level}, ${application.referral_source || ""}, ${application.anything_else},
      ...
      ${score.score_band}, ${score.score_version}, ${JSON.stringify(score.item_scores)}::jsonb, ${score.automatic_summary},
    ...
    on conflict (email) do update set
      ...
      scenario_frequency = excluded.scenario_frequency,
      ...
      visit_comfort = excluded.visit_comfort,
