...
The Reddit page-visit event fires after V8 or V9 records its first-party page view. The Reddit sign-up event fires only after `/api/waitlist` successfully saves the address, not on a button click or failed submission. Neither event sends form fields or application answers to the GTM data layer. The direct Meta integration sends `PageView`, plus a `Lead` event only after `/api/waitlist` successfully saves an address and marketing consent remains granted. The Meta event contains only the fixed labels `HarborNavi Waitlist` and `Email Signup`; it does not send waitlist fields or application answers. Its `<noscript>` fallback is intentionally omitted because that image request cannot honor the site's marketing-consent gate.

Use GTM Preview to confirm that no container request occurs after Decline, then allow marketing cookies and confirm both custom events. Publish the container and verify `PageVisit` and `SignUp` in Reddit Events Manager and Reddit Pixel Helper. For Meta Test Events, open `https://harbornavi.com/?meta_test=1`, select **Allow marketing cookies**, and confirm `PageView`; then submit a real, authorized waitlist address to confirm `Lead`. The test query only reopens the consent choice when needed and never grants consent automatically. A prior Decline choice intentionally keeps Meta off until that setting changes. Meta's `test_event_code` applies to server-side Conversions API payloads and is not a browser Pixel parameter. Browser events use `trackSingle` for Pixel `1257319255516210`, with a non-PII event ID reserved for future Pixel/CAPI deduplication. Do not add a second `All Pages` Reddit Page Visit tag or a second Meta base pixel in GTM, because either would duplicate page visits. `PUBLIC_REDDIT_PIXEL_ID` is the separate direct-pixel path for the other public pages and should remain empty when Reddit tracking is intended to run only through the V8/V9 GTM container. Both direct-pixel components honor the same consent cookie and stay off without `granted` consent.
